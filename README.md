...
## 亚马逊 A+ 详情页生成（/amazon-aplus）

基于 Gemini 文本 + 图片模型，帮助卖家一键生成亚马逊 A+ 页面内容。

**当前 MVP 已支持：**

| 步骤 | 说明 |
|------|------|
| Step 1 填写信息 | 品牌、产品名、3 条核心卖点（必填）、品牌故事（选填）、参考产品图（选填）、视觉风格、文案语言、生图模型 |
| Step 2 确认文案 | AI 生成的全套文案可在线编辑，确认后再生图（节省积分） |
| Step 3 生成图片 | 顺序生成 4 张图（Hero 16:9 × 1、特点图 1:1 × 3），自动存图库 |
| Step 4 查看结果 | 图文一一对应展示，所有文案一键复制，图片可下载 |

**支持语言**：英/德/法/意/西班牙/日/中/荷/瑞典/波兰
**支持风格**：白底简洁 / 场景生活 / 高端黑金
**接口**：`POST /api/amazon-aplus/analyze`（文案，不扣积分）+ `POST /api/amazon-aplus/generate`（生图，扣积分）

**完整方案（MVP 之外，待后续迭代）**：A/B 对比模块、产品规格图、场景生活大图、多语言批量生成、自定义模块顺序、导出为 HTML/ZIP、图片精准 px 裁剪。详见 `docs/KNOWN_ISSUES.md`。

---

## 修改图片（/image-edit）

基于 Gemini 图片编辑能力，支持 9 种模式：
...
- 后端接口 `POST /api/image-edit`，支持 Nano Banana / 2 / Pro，含重试逻辑。
- 修改后图片自动存图库，显示积分消耗。
- 文字替换/翻译支持字体样式（字重/颜色/字号放大缩小+百分比）。
- 每种模式顶部展示 before → after 可视化示例（`ModeDemo` 组件）。
- 演示图存于 `public/demo-*.png`（14 张）。
...
- A+ 模块完整方案（对比图、规格图、导出 ZIP 等，见 `docs/KNOWN_ISSUES.md`）
