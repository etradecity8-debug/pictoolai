...
- 在任意网页对图片使用右键菜单：**PicToolAI →** AI 美工 7 项 + **通用电商生图**子菜单（白底主图 / 特写图 / 卖点图 / 场景图 / 交互图）
- 自动打开 `pictoolai.studio` 对应页面并载入该图；生成结果请在站内 **保存到本地** 或从仓库下载，再上传到电商平台 / ERP。
...
2. **权限说明（供审核 / 隐私政策）**：
   ...
   - `scripting`：在来源标签页为目标 `<img>` 打标记（扩展流程关联来源图）。
   - `tabs`：定位来源标签页并注入脚本。
...
- 仅支持右键上下文为图片的 `<img>`；CSS `background-image`、Shadow DOM 内图片等可能无法标记。
- 上架用图以各平台 / ERP 后台实际上传的文件为准；PicToolAI 不自动写回店小秘等第三方后台。
