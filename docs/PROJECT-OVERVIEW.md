...
| 后端 | Node.js、Express、JWT、bcrypt、SQLite（`server/db.js` + `picaitool.db`） |
...
| 功能 | 路由/入口 | 说明 |
|------|-----------|------|
| 全品类组图 | 产品组图 | 上传图 + 组图要求 → 分析 → 确认规划 → 生图，详见 `DATA-FLOW.md` |
...
| 亚马逊 Listing | AI 运营助手 → 亚马逊 | Step1 分析 → Step2 标题/五点/描述/关键词 → Step3 主图 → Step4 A+ 文案与图，合规见 `AMAZON-LISTING-SPEC.md` |
...
- 数据库：首次使用自动创建 `server/picaitool.db`；备份复制 `picaitool.db` 与 `server/gallery/` 即可。
