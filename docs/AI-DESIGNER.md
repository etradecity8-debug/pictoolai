...
本文档说明 AI 美工（`/ai-designer`）下**全部功能**：图片编辑（局部重绘、局部消除、一键换色、**服装3D**、**服装平铺**、**调整身材**、**生成场景**、**添加人/物**）、质量提升（智能扩图、提升质感）、文字修改（文字替换、语言转换、**文字去除**）、**风格变迁**（**风格复刻**、**风格改变**）、**水印**（添加水印、AI 去除水印）、官方示例（7 种修改图片模式）。

**路由**：`/ai-designer`、`/ai-designer/:toolId`。侧边栏分为：图片编辑、质量提升、**文字修改**（文字替换、语言转换、文字去除）、**风格变迁**（风格复刻、风格改变）、**水印**（添加水印、去除水印）、官方示例。

**从仓库进入**：仓库页每张图提供「用AI编辑」按钮，点击后弹窗以**一二级菜单**（与侧栏一致：图片编辑、质量提升、文字修改、风格变迁、水印、官方示例）选择功能，跳转到对应页面并**自动将该图加载为待编辑图片**（`initialImageFromGallery` + `src/lib/loadGalleryImage.js`），无需再次上传。

**输出设置（与官方示例一致）**：局部重绘、局部消除、一键换色、智能扩图、提升质感、服装3D、服装平铺、调整身材、生成场景、添加人/物均提供「输出设置」折叠区：**模型**（可选 Nano Banana 2 / Pro / Nano Banana）、**输出尺寸比例**、**清晰度**，以及本次预计消耗积分。**默认模型**：除语言转换（默认 Nano Banana 2）外，其余模块均为 **Nano Banana**。组件 `src/components/OutputSettings.jsx`，请求时传 `model`、`aspectRatio`、`clarity`，后端优先使用请求参数。

---

## UI 布局与广告词（2026-03 统一）

全部功能模块采用统一排版规范：标题 `text-2xl font-bold`，副文案 `text-base text-gray-600`，示例图 `w-72 h-72` 或 `w-56 h-56`、`rounded-lg`，主内容卡片 `max-w-3xl mx-auto p-4`，主操作按钮 `max-w-xs mx-auto py-2.5 rounded-lg text-sm`，保存按钮紧贴生成结果放置。

**各模块广告词（与前端一致）**：

| 分类 | 模块 | 第一行（标题） | 第二行（副文案） |
|------|------|----------------|------------------|
| 图片编辑 | 局部重绘 | 局部重绘 | 上传图片，涂抹需要改变的区域并输入想要改变的内容，即可重新对该区域进行绘制 |
| 图片编辑 | 局部消除 | 局部消除 | 上传需要消除的图片，使用画笔涂抹即可实现一键消除 |
| 图片编辑 | 文字去除 | 文字去除 | 上传含文字的图片，使用画笔涂抹需要去除的文字区域，一键清除 |
| 图片编辑 | 一键换色 | 一键换色 | 无论是时装还是产品，一键精准换色，效率提升 10 倍 |
| 图片编辑 | 服装3D | 服装 3D | 上传平铺服装图片，一键生成立体 3D 展示效果 |
| 图片编辑 | 服装平铺 | 服装平铺 | 上传服装图片，自动生成带配饰的精美平铺展示图 |
| 图片编辑 | 调整身材 | 调整身材 | 上传模特穿搭图，调整模特体重，展示服装在不同身材上的效果 |
| 图片编辑 | 生成场景 | 生成场景 | 一张产品图，千种生活场景 —— 让买家看见拥有它的样子 |
| 图片编辑 | 添加人/物 | 添加人/物 | 在照片上指定添加人物或物品，并选择放置位置 |
| 质量提升 | 智能扩图 | 智能扩图，视界大开。 | 不再受限于快门的瞬间，PicToolAI 自动识别你的构图，智能补全缺失的肩膀、延伸的花海、广阔的星空。第二行：每一像素的扩展，都与原图浑然一体。 |
| 质量提升 | 提升质感 | 提升质感 | 内置专业材质引擎，一键让你的产品图从「地摊货」变身「奢侈品」。 |
| 风格变迁 | 风格改变 | 风格改变 | 上传一张照片，选择目标风格（如二次元、日漫、水彩等），一键生成该风格下的新图 |
| 水印 | 添加水印 | 添加水印 | 上传图片，设置水印文字、位置与透明度，一键生成带水印图（本地处理，不消耗积分） |
| 水印 | 去除水印 | 去除水印 | AI 去除画面中的水印与叠加层，智能填补背景（扣积分） |
...
4. **推荐模型**：扩图对全局逻辑、背景补全要求高，用户可切换 Nano Banana 2 获得更好效果。

### 当前实现

- **默认模型**：Nano Banana（用户可在输出设置中切换，推荐扩图场景优先尝试 Nano Banana 2）
- **第一步（视觉分析）**：使用 `ANALYSIS_MODEL_ID`（gemini-2.5-flash）分析原图，提取 5–10 个关键词（主体、背景元素、光照风格）
- **第二步（生图）**：将关键词注入 `[CONTENT TO ADD] Extend the following elements: {keywords}.` 模板，调用所选模型生图
...
**默认模型**：Nano Banana，用户可在输出设置中切换 Pro 或 Nano Banana 2。
...
侧边栏「风格变迁」组下含「风格复刻」与「风格改变」，该组位于「文字修改」与「水印」之间。**风格复刻**：上传参考设计图（最多 14 张）与产品素材图（最多 6 张），AI 两阶段生成：① 分析参考图风格；② 按产品图 + 风格描述生图。
...
侧栏统一为 **「水印」** 大类，便于客户在同一处完成「叠加文字水印」与「AI 去除水印」（实现方式不同，见下）。

## 5.1 添加水印

侧边栏「水印 → 添加水印」，路由 `/ai-designer/watermark-add`。
...
侧边栏「水印 → 去除水印」，路由 `/ai-designer/watermark-remove`。页面仍由 `ImageEdit` 组件承载（与其它「官方示例」模式相同的技术栈），仅导航归类在「水印」下。

- **接口**：`POST /api/image-edit`，`mode: 'watermark-remove'`
- **说明**：去除图片中的水印与叠加层并智能填补背景；**消耗积分**，有 before/after 官方示例图。
- **默认模型**：Nano Banana（可在输出设置中切换）

---

# 六、官方示例（7 种修改图片模式）

AI 美工侧边栏「官方示例」下提供 7 种修改图片模式（**不含**去除水印；去除水印见上文「五、水印」），与 `ImageEdit` 组件一致，均调用 `POST /api/image-edit`。每种模式有真实 before→after 示例（`ModeDemo` 组件）。**默认模型**：Nano Banana。
