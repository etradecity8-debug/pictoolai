...
本文档说明 AI 美工（`/ai-designer`）下**全部功能**：图片编辑（局部重绘、局部消除、一键换色）、质量提升（智能扩图、提升质感）、官方示例（9 种修改图片模式）。

**路由**：`/ai-designer`、`/ai-designer/:toolId`。侧边栏分为：图片编辑、质量提升、图像修复、抠图工具、官方示例。
...
AI 美工侧边栏「官方示例」下提供 9 种修改图片模式，与主导航「修改图片」功能相同，均调用 `POST /api/image-edit`。每种模式有真实 before→after 示例（`ModeDemo` 组件）。
...
| `text-replace` | 图片文字替换 | 替换图中的文字内容 |
| `text-translate` | 图片文字语言转换 | 将图中文字翻译为另一语言 |
