...
| 水印 | 水印添加 | 添加水印 | 上传图片，设置水印文字、位置与透明度，一键生成带水印图（本地处理，不消耗积分） |
...
AI 美工侧边栏「官方示例」下提供 7 种修改图片模式（与风格复刻同级），与主导航「修改图片」功能相同，均调用 `POST /api/image-edit`。每种模式有真实 before→after 示例（`ModeDemo` 组件）。**默认模型**：Nano Banana。
