...
本文档涵盖「电商 AI 运营助手」全平台内容：**亚马逊**（Listing 生成/优化/竞品/关键词）、**eBay**（生成/优化）、**速卖通**（生成/优化）。**侵权风险检测**已拆分为独立主导航模块，详见 [IP-RISK.md](./IP-RISK.md)。

> **说明**：**A+ 模块（生成 Listing Step 4 + 独立 A+ 详情页）暂不实现**，界面与导航均已隐藏，详见 [KNOWN_ISSUES.md](./KNOWN_ISSUES.md)。
...
| Step 4 | **A+ 文案与图片**（可选） | 以上全部结果 | 约 2–3K tokens + 4 张 A+ 图；**当前已锁定**，界面显示「功能优化中」，详见 `KNOWN_ISSUES.md` |

> 注：Step 3 产品图与 Step 4 A+ 可并行；A+ 恢复方式见 `KNOWN_ISSUES.md`。
...
侵权风险检测已拆分为独立主导航模块（路由 `/ip-risk`），功能、费用、漏检分析、外部数据源可行性详见 [IP-RISK.md](./IP-RISK.md)。
