...
本文档仅涵盖「**电商 AI 运营助手**」内能力：**亚马逊**（Listing 生成/优化/竞品/关键词）、**eBay**（生成/优化）、**速卖通**（生成/优化）。站内其它独立模块见 [文档总索引](./README.md)，**不在本文展开**。

> **前台可见性**：**A+**（生成 Listing Step 4 + 独立页 `/amazon-aplus`）由 `SITE_NAV_HIDDEN.amazonAplus` 控制是否展示入口；隐藏时路径重定向首页，**本文档中的 Listing / A+ 产品设计仍有效**。全站隐藏入口总表见 [TEMPORARILY-HIDDEN-FEATURES.md](./TEMPORARILY-HIDDEN-FEATURES.md)。A+ 效果因模块与品牌要求而异，另见 [KNOWN_ISSUES.md](./KNOWN_ISSUES.md) 1.1。
...
| 大分类 | 小分类 |
|--------|--------|
| [一、亚马逊](#一亚马逊) | 1.1 生成 Listing · 1.2 生图提示词 · 1.3 A+ 模块定义 · 1.4 优化 Listing · 1.5 竞品对比 · 1.6 关键词研究 |
| [二、eBay](#二ebay) | 2.1 平台规格 · 2.2 流程 · 2.3 优化 Listing · 2.4 数据存储与接口 |
| [三、速卖通](#三速卖通aliexpress) | 3.1 平台规格 · 3.2 速卖通规则 · 3.3 流程 · 3.4 数据存储与接口 |
...
| Step 2 | **标题 + 后台关键词 + 五点 + 产品描述** | Step1 结果 + 用户输入 | 约 1–1.5K tokens |
| Step 3 | **产品图**（可选） | 本模块内生成 | 走图像生成流程 |
| Step 4 | **A+ 文案与图片**（可选） | 以上全部结果 | 约 2–3K tokens + 按所选模块张数生图；扣积分规则以页面为准 |

> 注：Step 3 产品图与 Step 4 A+ 可并行。
...
### 1.2.2 Step 3 产品图（白底主图 / 场景图 / 特写图 / 卖点图）
