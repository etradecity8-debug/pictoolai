...
- `generate-listing`：prompt 已按第七章 7.1–7.3、7.4 全面约束；并已按 **A9 / Cosmo / Rufus / GEO** 分别写入可执行规则（见 `server/index.js` 中 generate-listing 的 prompt）：**标题**（≤200、前 80 字核心词、同词不超 2 次、禁止 `! $ ? _ { } ^ ¬ ¦`、禁止全大写/竞品/堆砌）；**五点**（5 条、每条 ≤500、建议 ≤255 以兼容部分类目、禁止价格/促销/站外/特殊符号/emoji/跨条重复/N/A/TBD/联系/质保/类目禁用词）；**描述**（≤2000、禁止与五点重复、禁止竞品/best seller 类宣称）；**searchTerms**（≤250 bytes、不重复标题与五点、逗号或空格分隔）。输出 JSON 后后端：标题去除上述禁用符号并截断至 200 字符；五点每条截断至 500；描述截断至 2000；searchTerms 按 UTF-8 字节截断至 250。
